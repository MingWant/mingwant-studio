---
name: open-canvas
description: 打开明想 MingWant Studio 网页画布并自动连接本地 Canvas Agent。用户要求打开、启动、进入、使用明想或画布时使用。
---

# 打开明想 MingWant Studio 画布

当用户要求打开、启动、进入或使用明想 MingWant Studio 时，不要把 URL 交给用户手动复制，不要通过浏览器点击“新建画布”。优先快速拉起本地画布和本地 Canvas Agent，然后直接打开带 `mode` 与凭据 fragment 的 URL，让网页自动创建或选择画布并连接 Agent。Token 禁止放进 `?query`，否则会进入网页服务器访问日志。
...
1. 如果当前仓库是明想 MingWant Studio 项目，优先使用当前仓库的 `web/` 前端。
2. 先检查本地端口归属：如果 `3000`、`3001` 等端口已被占用，必须确认监听进程的工作目录属于当前仓库的 `web/`，不能只因为端口存在就当成本地画布。Windows 用 `Get-NetTCPConnection -State Listen -LocalPort <端口>` 配合 `Get-Process -Id <PID>`；macOS/Linux 才使用 `lsof`/`ps` 或服务输出。
...
7. 不走本地 Agent 的 `/open` 跳转；对地址与 Token 分别做 URL 编码后，直接构造并打开最终 URL：`<真实画布地址>/canvas?mode=new#agentUrl=<Local URL>&agentToken=<Connect token>`。`#` 后内容不会发送给画布服务器，网页读取后会立即清除。
...
- 如果当前工作区不是明想 MingWant Studio 源码仓库，优先提示用户先打开或启动明想网页，再连接本地 Agent。
