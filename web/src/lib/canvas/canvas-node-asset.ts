...
export function declaredCanvasNodeAssetCategory(node: CanvasNodeData): AssetCategory | undefined {
    ...
    if (node.metadata?.workflowKind === "commerce_identity") return "character";
    if (node.metadata?.workflowKind === "commerce_product") return "product";
    if (node.metadata?.workflowKind === "commerce_rights") return "rights";
    return undefined;
}
