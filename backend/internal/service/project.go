...
type ProjectSummary struct {
	...
	WorkflowStepCount          int           `json:"workflowStepCount"`
	CompletedWorkflowStepCount int           `json:"completedWorkflowStepCount"`
}
...
func (s *Service) ListProjects(userID string) ([]ProjectSummary, error) {
	...
	for _, project := range projects {
		...
		workflows, workflowsErr := s.ProjectWorkflows(project.ID)
		if workflowsErr != nil {
			return nil, workflowsErr
		}
		workflowStepCount := 0
		completedWorkflowStepCount := 0
		for _, workflow := range workflows {
			workflowStepCount += len(workflow.Steps)
			for _, step := range workflow.Steps {
				if step.Status == model.WorkflowStepStatusCompleted || step.Status == model.WorkflowStepStatusSkipped {
					completedWorkflowStepCount++
				}
			}
		}
		result = append(result, ProjectSummary{Project: project, CanvasCount: len(canvases), AssetCount: assetCount, UnitCount: len(units), CompletedUnitCount: completed, WorkflowStepCount: workflowStepCount, CompletedWorkflowStepCount: completedWorkflowStepCount})
	}
	return result, nil
}
