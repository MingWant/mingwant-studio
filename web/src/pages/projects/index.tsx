...
type ProjectForm = { name: string; type: string; aspectRatio: string; sourceType: string };
    ...
    return (
        <WorkspacePage>
            <PageHeader
                icon="projects"
                title="制作项目"
                description="统一管理 AI 电商带货、商业短视频与短剧项目，按工作流继续生产。"
            ...
            <ListToolbar active={Boolean(keyword || status !== "all" || sort !== "updated")} onReset={() => { setKeyword(""); setStatus("all"); setSort("updated"); }}>
                ...
                <Select className="w-32" value={sort} onChange={setSort} options={[{ label: "最近更新", value: "updated" }, { label: "制作进度", value: "progress" }, { label: "项目名称", value: "name" }]} />
            </ListToolbar>
            ...
            {query.isLoading ? <WorkspaceLoadingState label="正在整理项目" detail="读取工作流、画布与资产进度" /> : null}
            {!query.isLoading && rows.length ? (
                <TableSurface>
                    <div className="hidden h-10 grid-cols-[minmax(240px,1.2fr)_minmax(150px,.75fr)_minmax(150px,.7fr)_minmax(180px,.8fr)_120px_28px] items-center gap-4 border-b border-border/70 bg-foreground/[.025] px-4 text-[11px] font-medium text-foreground/42 lg:grid">
                        <span>项目</span><span>当前阶段</span><span>制作进度</span><span>内容</span><span>最近更新</span><span />
                    </div>
            ...
            {!query.isLoading && !rows.length && !query.isError ? (
                <WorkspaceState
                    icon="projects"
                    title={keyword || status !== "all" ? "没有匹配的项目" : "创建第一个制作项目"}
                    description={keyword || status !== "all" ? "调整搜索词或状态筛选后再试。" : "项目会集中保存素材、画布、分镜、生成结果和制作进度。自由创作可从画布开始。"}
            ...
            <Modal title="创建制作项目" open={createOpen} footer={null} destroyOnHidden onCancel={() => setCreateOpen(false)} width={520} styles={{ body: { paddingTop: 12 } }}>
                <Form<ProjectForm> layout="vertical" initialValues={{ type: "commerce-video", aspectRatio: "9:16", sourceType: "talent-videos" }} onFinish={(values) => mutation.mutate(values)}>
                    <Form.Item name="name" label="项目名称" rules={[{ required: true, whitespace: true, message: "请输入项目名称" }]}><Input autoFocus placeholder="例如：夏季新品 15 条带货视频" /></Form.Item>
                    <Form.Item name="type" label="项目类型">
                        <Select options={[{ label: "AI 电商带货 / 商业短视频", value: "commerce-video" }, { label: "AI 短剧 / 故事视频", value: "short-drama" }]} />
                    </Form.Item>
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <Form.Item name="sourceType" label="内容来源"><Select options={[{ label: "5 条真人授权素材", value: "talent-videos" }, { label: "品牌商品素材", value: "product-assets" }, { label: "空白开始", value: "blank" }, { label: "导入小说", value: "novel" }, { label: "粘贴文本", value: "text" }]} /></Form.Item>
                    </div>
                    <p className="-mt-1 mb-5 text-xs leading-5 text-foreground/48">电商项目会自动使用 Brief、授权、身份、商品、创意矩阵、首尾帧、生成、质检与交付流程；短剧项目保留原标准流程。</p>
...
function ProjectRow({ row }: { row: ProjectSummary }) {
    ...
    const unitLabel = row.project.type === "commerce-video" ? "步骤" : "章节";
    const completedCount = row.project.type === "commerce-video" ? row.completedWorkflowStepCount : row.completedUnitCount;
    const totalCount = row.project.type === "commerce-video" ? row.workflowStepCount : row.unitCount;
    return (
...
function projectTypeLabel(value: string) {
    return value === "commerce-video" ? "电商短视频" : value === "short-drama" ? "短剧" : "制作项目";
}
