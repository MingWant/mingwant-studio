...
export function projectSummaryCompletion(summary: ProjectSummary) {
    if (summary.project.type === "commerce-video") return summary.workflowStepCount ? Math.round((summary.completedWorkflowStepCount / summary.workflowStepCount) * 100) : 0;
...
export function projectSummaryStage(summary: ProjectSummary) {
    ...
    if (summary.project.type === "commerce-video") {
        if (summary.workflowStepCount && summary.completedWorkflowStepCount === summary.workflowStepCount) return { label: "交付完成", detail: "九步电商生产工作流已完成" };
        if (!summary.canvasCount) return { label: "准备素材", detail: "下一步创建 15 条带货生产画布" };
        if (!summary.assetCount) return { label: "锁定参考", detail: "补充艺人与商品准确度素材" };
        if (summary.unitCount && summary.completedUnitCount === summary.unitCount) return { label: "待交付", detail: "创意条目已完成，可执行最终质检" };
        return { label: "商业视频制作中", detail: `${summary.canvasCount} 张画布 · ${summary.assetCount} 项资产` };
    }
...
export function projectDetailStage(detail: ProjectDetail) {
    ...
    if (detail.project.type === "commerce-video") {
        const steps = detail.workflows.flatMap((workflow) => workflow.steps);
        const activeStep = steps.find((step) => step.status === "running" || step.status === "review" || step.status === "ready");
        if (activeStep) return { label: activeStep.name, detail: "按电商生产工作流继续推进" };
        if (steps.length && steps.every((step) => step.status === "completed" || step.status === "skipped")) return { label: "交付完成", detail: "全部电商工作流步骤已完成" };
        return { label: "准备电商画布", detail: "导入授权素材并建立 15 条创意矩阵" };
    }
...
export function projectContinueTarget(detail: ProjectDetail): ProjectContinueTarget {
    ...
    if (detail.project.type === "commerce-video") {
        return {
            href: `/projects/${detail.project.id}/canvases`,
            title: detail.project.name,
            context: "创建 15 条带货生产画布",
...
export function projectNextActions(detail: ProjectDetail, limit = 4): ProjectWorkbenchAction[] {
    ...
    if (detail.project.type === "commerce-video") return commerceProjectNextActions(detail, limit);
...
function commerceProjectNextActions(detail: ProjectDetail, limit: number): ProjectWorkbenchAction[] {
    const actions: ProjectWorkbenchAction[] = [];
    const projectRoot = `/projects/${detail.project.id}`;
    const failedSteps = detail.workflows.flatMap((workflow) => workflow.steps).filter((step) => step.status === "failed");
    if (failedSteps.length) {
        actions.push({
            id: "commerce-failed-steps",
            title: `处理 ${failedSteps.length} 个失败步骤`,
            description: failedSteps[0].error?.trim() || "核对输入、模型与素材后，仅重试受影响的任务。",
            href: "/tasks?status=failed",
            actionLabel: "查看失败任务",
            tone: "danger",
        });
    }
    if (!detail.canvases.length) {
        actions.push({
            id: "commerce-canvas",
            title: "创建 15 条带货生产画布",
            description: "使用 5 条真人素材 × 3 个卖点的预置工作流，展开分镜、首尾帧、成片与 QA。",
            href: `${projectRoot}/canvases`,
            actionLabel: "打开并创建",
            tone: "default",
        });
    ...
    if (!detail.assets.length) {
        actions.push({
            id: "commerce-assets",
            title: "补充授权与商品参考素材",
            description: "先锁定艺人身份、商品包装、Logo、规格和可使用的广告宣称。",
            href: `${projectRoot}/assets`,
            actionLabel: "整理素材",
            tone: "attention",
        });
    }
    const activeStep = detail.workflows.flatMap((workflow) => workflow.steps).find((step) => step.status === "ready" || step.status === "running" || step.status === "review");
    if (activeStep) {
        actions.push({
            id: `commerce-step-${activeStep.id}`,
            title: activeStep.name,
            description: "按项目工作流记录输入、生成结果、质检结论和审批状态。",
            href: `${projectRoot}/overview`,
            actionLabel: "继续工作流",
            tone: activeStep.status === "review" ? "attention" : "default",
        });
    }
    if (!actions.length) {
        const target = projectContinueTarget(detail);
        actions.push({
            id: "commerce-continue",
            title: "继续最近的商业视频内容",
            description: `${target.context}：${target.title}`,
            href: target.href,
            actionLabel: "继续制作",
