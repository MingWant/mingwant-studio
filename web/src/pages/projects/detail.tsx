...
export default function ProjectDetailPage() {
    ...
    const createCanvas = () => {
        ...
        const initialContent = commerceProject ? createCommerceWorkflowTemplate() : seed ? { nodes: seed.nodes, connections: seed.connections } : undefined;
        const title = commerceProject ? `${detail.data?.project.name || "电商项目"} · 15 条带货工作流` : unit ? `${unit.title} · ${shots.length ? "分镜画布" : "画布"}` : `${detail.data?.project.name || "项目"} · 新画布`;
        void createCanvasProjectWithRemoteSync(title, projectId, initialContent).then(async ({ id, syncError }) => {
            ...
            refreshProject();
            message.success(commerceProject ? "15 条带货工作流画布已创建并关联项目" : unit && shots.length ? `已创建章节画布并导入 ${shots.length} 个分镜` : unit ? "章节画布已创建并关联" : "项目画布已创建");
            navigate(`/canvas/${id}`);
    ...
    const commerceProject = detail.data.project.type === "commerce-video";
    if (commerceProject && activeView === "chapters") return <Navigate to={`/projects/${projectId}/overview`} replace />;
    if (!chapterId && (!view || !views.some((item) => item.key === view))) return <Navigate to={`/projects/${projectId}/overview`} replace />;
    const visibleViews = commerceProject ? views.filter((item) => item.key !== "chapters") : views;
                ...
                <header className="shrink-0 border-b border-border/65 bg-background/80 px-3 py-2 backdrop-blur-md sm:px-4 lg:px-5 lg:py-0">
                    <div className="flex min-w-0 flex-wrap items-center gap-x-3 lg:min-h-16 lg:flex-nowrap">
                        ...
                        <nav className="thin-scrollbar order-last mt-1 flex h-11 w-full min-w-0 items-center gap-0.5 overflow-x-auto lg:order-none lg:mt-0 lg:h-16 lg:flex-1 lg:border-l lg:border-border/55 lg:pl-3" aria-label="项目导航">
                            {visibleViews.map((item) => { const Icon = item.icon; const active = item.key === activeView; const href = item.key === "chapters" ? chapterHref : `/projects/${projectId}/${item.key}`; const label = commerceProject && item.key === "assets" ? "人物与商品素材" : item.label; return <Link key={item.key} to={href} className={`relative flex h-11 shrink-0 items-center gap-2 rounded-md px-2.5 text-[13px] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:px-3 ${active ? "bg-[var(--workspace-accent-soft)] font-medium text-foreground lg:after:absolute lg:after:inset-x-3 lg:after:bottom-0 lg:after:h-0.5 lg:after:rounded-full lg:after:bg-[var(--workspace-accent)]" : "text-foreground/52 hover:bg-foreground/[.045] hover:text-foreground"}`} aria-current={active ? "page" : undefined}><Icon className={`size-4 shrink-0 ${active ? "text-[var(--workspace-accent)]" : "text-foreground/45"}`} /><span className="sm:hidden">{item.shortLabel}</span><span className="hidden sm:inline">{label}</span></Link>; })}
                        </nav>
                        <Tooltip title={commerceProject ? "创建并关联 15 条带货工作流" : activeView === "chapters" && detail.data.units.length ? "新建当前章节画布" : "新建项目画布"}><Button size="small" className="!h-9 !shrink-0 !px-2 sm:!px-3" icon={<Plus className="size-4" />} onClick={createCanvas} aria-label={commerceProject ? "新建带货工作流画布" : activeView === "chapters" && detail.data.units.length ? "新建当前章节画布" : "新建项目画布"}><span className="hidden sm:inline">{commerceProject ? "带货画布" : "新建画布"}</span></Button></Tooltip>
                    </div>
                </header>
