...
import { sanitizeCanvasStoryboardNode } from "@/lib/canvas/canvas-storyboard-text";
...
export function useCanvasProjectLifecycle({
    ...
    useEffect(() => {
        ...
        const restore = async () => {
            const initialNodes = resetInterruptedGeneration(project.nodes).map(sanitizeCanvasStoryboardNode);
    ...
    useEffect(() => {
        ...
        listActivatedSkills()
            .then((result) => {
                if (!cancelled) setActivatedSkills(Array.isArray(result?.skills) ? result.skills : []);
            })
            .catch(() => {
