...
import type { WorkflowStep } from "@/services/api/projects";
...
export default function ProjectOverviewView({ detail }: ProjectDetailViewProps) {
    ...
    const commerceProject = project.type === "commerce-video";
    const workflowSteps = detail.workflows.flatMap((workflow) => workflow.steps);
    const completedUnits = units.filter((unit) => unit.status === "completed").length;
    const completedWorkflowSteps = workflowSteps.filter((step) => step.status === "completed" || step.status === "skipped").length;
    const attentionCount = projectAttentionCount(detail);
    const completion = commerceProject ? workflowSteps.length ? Math.round((completedWorkflowSteps / workflowSteps.length) * 100) : 0 : units.length ? Math.round((completedUnits / units.length) * 100) : 0;
    ...
    return (
        <div className="space-y-7">
                ...
                <div className="grid lg:grid-cols-[minmax(0,1fr)_300px]">
                    ...
                    <aside className="border-t border-border/70 bg-foreground/[.018] p-5 lg:border-l lg:border-t-0 lg:p-6" aria-label="项目进度">
                        <div className="flex items-end justify-between gap-3">
                            <div><div className="text-[10px] font-medium text-foreground/38">{commerceProject ? "工作流进度" : "章节进度"}</div><div className="mt-1 text-lg font-semibold tabular-nums">{commerceProject ? completedWorkflowSteps : completedUnits}<span className="mx-1 text-sm font-normal text-foreground/28">/</span>{commerceProject ? workflowSteps.length : units.length}</div></div>
                            <span className="text-xs font-medium tabular-nums text-foreground/42">{completion}%</span>
                        </div>
                        <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-foreground/[.08]" aria-label={`${commerceProject ? "工作流" : "章节"}完成度 ${completion}%`}><div className="h-full rounded-full bg-[var(--workspace-accent)] transition-[width]" style={{ width: `${completion}%` }} /></div>
                        <dl className="mt-5 grid grid-cols-2 gap-x-5 gap-y-3 text-xs">
                            <ProjectFact label="当前阶段" value={stage.label} />
                            <ProjectFact label={commerceProject ? "工作流步骤" : "分镜镜头"} value={`${commerceProject ? workflowSteps.length : shots.length} 个`} />
            ...
            {commerceProject ? <CommerceWorkflowSection projectId={project.id} steps={workflowSteps} canvasCount={canvases.length} /> : <section>
...
function CommerceWorkflowSection({ projectId, steps, canvasCount }: { projectId: string; steps: WorkflowStep[]; canvasCount: number }) {
    return (
        <section>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div className="text-[10px] font-medium text-foreground/38">AI 电商生产流水线</div>
                    <h2 className="mt-1 text-lg font-semibold">从授权到交付</h2>
                    <p className="mt-1 text-xs leading-5 text-foreground/46">九步状态由后端工作流持久化；15 条脚本、首尾帧与成片在关联画布中生产。</p>
                </div>
                <Link to={`/projects/${projectId}/canvases`} className="inline-flex h-8 items-center gap-1.5 text-xs font-medium text-foreground/48 hover:text-foreground">{canvasCount ? "查看项目画布" : "创建带货画布"}<ArrowRight className="size-3.5" /></Link>
            </div>
            {steps.length ? (
                <div className="mt-4 overflow-hidden rounded-lg border border-border/75 bg-background/70">
                    <div className="grid gap-px bg-border/65 sm:grid-cols-2 xl:grid-cols-3">
                        {steps.map((step, index) => (
                            <article key={step.id} className="flex min-h-24 gap-3 bg-background p-4">
                                <span className={`grid size-8 shrink-0 place-items-center rounded-md text-[10px] font-semibold tabular-nums ${step.status === "completed" ? "bg-emerald-500/12 text-emerald-600 dark:text-emerald-400" : step.status === "failed" ? "bg-red-500/12 text-red-600 dark:text-red-400" : step.status === "running" || step.status === "review" || step.status === "ready" ? "bg-[var(--workspace-accent-soft)] text-[var(--workspace-accent)]" : "bg-foreground/[.055] text-foreground/38"}`}>{String(index + 1).padStart(2, "0")}</span>
                                <span className="min-w-0"><strong className="block text-sm font-medium">{step.name}</strong><span className="mt-1.5 block text-[10px] text-foreground/42">{workflowStatusLabel(step.status)}</span>{step.error ? <span className="mt-1 line-clamp-2 block text-[10px] leading-4 text-red-500">{step.error}</span> : null}</span>
                            </article>
                        ))}
                    </div>
                </div>
            ) : <div className="mt-4 overflow-hidden rounded-lg border border-border/75 bg-background/70 p-2"><WorkspaceState icon="projects" compact title="工作流尚未初始化" description="刷新项目；若仍未出现，请检查后端内置工作流模板。" /></div>}
        </section>
    );
}

function workflowStatusLabel(status: string) {
    const labels: Record<string, string> = { pending: "等待前置步骤", ready: "可以开始", running: "执行中", review: "等待审核", completed: "已完成", failed: "失败，需处理", skipped: "已跳过" };
