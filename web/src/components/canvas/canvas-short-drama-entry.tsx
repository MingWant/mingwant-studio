...
export function CanvasShortDramaGuide({ progress, collapsed, onToggle, onSkip, onStepClick }: {
    ...
    if (collapsed) {
        return (
            <button type="button" data-canvas-no-zoom className="absolute left-[calc(50%+64px)] top-[104px] z-[48] inline-flex h-8 max-w-[calc(50vw-76px)] min-w-0 items-center gap-2 rounded-lg border px-3 text-xs font-medium shadow-sm backdrop-blur outline-none focus-visible:ring-2 md:top-2" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text, "--tw-ring-color": theme.accent.primary } as CSSProperties} onClick={onToggle}>
    ...
    return (
        <div data-canvas-no-zoom className="absolute left-1/2 top-[104px] z-[48] flex max-w-[calc(100%_-_24px)] -translate-x-1/2 items-center gap-1 rounded-lg border p-1 shadow-sm backdrop-blur md:top-[68px]" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }}>
