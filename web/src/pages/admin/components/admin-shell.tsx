...
import { BarChart3, BellRing, Coins, FileClock, HardDrive, Home, Mail, MessageSquareText, PanelLeftClose, PanelLeftOpen, RadioTower, Settings2, ShieldCheck, Sparkles, TicketCheck, UsersRound } from "lucide-react";
        ...
        <main className="app-user-workspace flex h-full min-h-0 overflow-hidden text-foreground">
            <aside className={cn("app-workspace-sidebar hidden shrink-0 flex-col overflow-hidden lg:flex", collapsed ? "w-[60px]" : "w-[212px]")}>
                <div className={cn("flex h-13 shrink-0 items-center", collapsed ? "justify-center" : "gap-2 px-3")}>
                    {!collapsed ? (
                        <Link to="/" className="flex min-w-0 flex-1 items-center gap-2" title="明想 MingWant Studio">
                            <span className="grid size-7 shrink-0 place-items-center rounded-md bg-foreground text-background"><Sparkles className="size-4" /></span>
                            <span className="min-w-0"><span className="block truncate text-[13px] font-semibold">明想 MingWant</span><span className="block truncate text-[9px] text-foreground/42">Studio 管理后台</span></span>
                        </Link>
                    ) : null}
