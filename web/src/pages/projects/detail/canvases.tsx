...
export default function ProjectCanvasesView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    const commerceProject = detail.project.type === "commerce-video";
    ...
    return (
        <div>
                    ...
                    {canvases.map((canvas, index) => {
                        ...
                        return (
                            <article key={canvas.id} className="min-w-0 overflow-hidden rounded-lg border border-border/80 bg-background">
                                ...
                                <div className="p-2.5">
                                    <div className="flex items-center justify-between"><span className="text-[10px] font-medium text-foreground/48">{commerceProject ? "画布用途" : "关联章节"}</span><span className="text-[9px] tabular-nums text-foreground/38">{commerceProject ? "项目级" : `${linkedUnits.length} 个`}</span></div>
                                    <div className="mt-1.5 flex min-h-6 max-h-12 flex-wrap gap-1 overflow-y-auto">
                                        {commerceProject ? <span className="py-0.5 text-[10px] text-foreground/55">15 条带货视频批量生产</span> : linkedUnits.length ? linkedUnits.map((unit) => (
                                    ...
                                    <div className="mt-1.5 flex items-center gap-1.5 border-t border-border/60 pt-2">
                                        {!commerceProject ? <Select size="small" className="min-w-0 flex-1" placeholder={unlinkedUnits.length ? "关联更多章节" : "全部章节已关联"} disabled={!unlinkedUnits.length} options={unlinkedUnits.map((unit) => ({ label: `${String(unit.position + 1).padStart(2, "0")} · ${unit.title}`, value: unit.id }))} onChange={(unitId) => { setLinkingCanvasId(canvas.id); linkMutation.mutate({ canvasId: canvas.id, unitId }); }} loading={linkMutation.isPending && linkingCanvasId === canvas.id} suffixIcon={<Link2 className="size-3.5" />} /> : <span className="mr-auto text-[10px] text-foreground/42">已关联当前电商项目</span>}
            ...
            ) : <WorkspaceState icon="canvas" title="还没有项目画布" description={commerceProject ? "使用右上角的“带货画布”创建完整 15 条生产工作流。" : "使用右上角的新建画布开始创作。"} />}
        </div>
    );
}
