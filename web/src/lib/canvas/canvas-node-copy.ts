...
export function isolateCopiedNodeMetadata(node: CanvasNodeData, idMap: ReadonlyMap<string, string>): CanvasNodeMetadata {
    ...
    // 图片标注和改图结果属于节点引用；连同源节点复制时重映射，否则继续引用画布上的原节点。
    metadata.imageAnnotation = node.metadata?.imageAnnotation ? {
        ...node.metadata.imageAnnotation,
        sourceNodeId: remapReferenceId(node.metadata.imageAnnotation.sourceNodeId, idMap)!,
        mask: { ...node.metadata.imageAnnotation.mask },
    } : undefined;
    metadata.imageAnnotationResultOf = remapReferenceId(node.metadata?.imageAnnotationResultOf, idMap);
