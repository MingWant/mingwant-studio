...
export default function ProjectSettingsView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    const sourceTypeOptions = project.type === "commerce-video"
        ? [{ label: "5 条真人授权素材", value: "talent-videos" }, { label: "品牌商品素材", value: "product-assets" }, { label: "空白开始", value: "blank" }]
        : [{ label: "空白开始", value: "blank" }, { label: "导入小说", value: "novel" }, { label: "粘贴文本", value: "text" }];
        ...
        <div>
            ...
            <section className="border-b border-border/70 py-4">
                ...
                <div className="grid gap-x-4 gap-y-3 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <Field label="默认画幅"><Select className="w-full" value={aspectRatio} options={[{ label: "9:16 · 竖屏", value: "9:16" }, { label: "16:9 · 横屏", value: "16:9" }, { label: "1:1 · 方形", value: "1:1" }]} onChange={setAspectRatio} /></Field>
                    <Field label="内容来源"><Select className="w-full" value={sourceType} options={sourceTypeOptions} onChange={setSourceType} /></Field>
            ...
            <section className="py-4">
                <div className="flex flex-col gap-3 rounded-lg border border-red-500/20 bg-red-500/[.025] px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex min-w-0 items-center gap-2.5"><span className="grid size-7 shrink-0 place-items-center rounded bg-red-500/10 text-red-500"><Archive className="size-3.5" /></span><div className="min-w-0"><h3 className="text-sm font-medium">{project.status === "archived" ? "恢复项目" : "归档项目"}</h3><p className="mt-0.5 text-[11px] text-foreground/48">{project.status === "archived" ? "恢复后可继续创建画布、内容单元和生成任务" : "保留全部画布、内容和资产，停止项目内新建与生成"}</p></div></div>
