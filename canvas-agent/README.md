# 明想 MingWant Studio Canvas Agent
...
使用时可以直接在 Codex 里说“打开明想 MingWant Studio”，插件会优先启动本地画布和本地 Agent，读取 Local URL 和 Connect token，然后直接打开画布网页地址新建并连接画布。如果自动连接失败，再检查本地画布服务和 Canvas Agent 是否都已启动。
...
```bash
codex mcp add infinite-canvas -- node /path/to/mingwant-studio/canvas-agent/dist/index.js mcp
```
...
```bash
claude mcp add --scope user --transport stdio infinite-canvas -- node /path/to/mingwant-studio/canvas-agent/dist/index.js mcp
```
