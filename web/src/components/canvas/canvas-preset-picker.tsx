...
import { BookOpenCheck, LoaderCircle, Search, WandSparkles } from "lucide-react";

import { canvasThemes } from "@/lib/canvas-theme";
import { loadMingWantPrompt, mingwantPromptTemplates } from "@/lib/mingwant-prompt-library";
...
export type CanvasPromptPreset = {
    ...
    source: "builtin" | "skill" | "mingwant";
};

type CanvasPromptPresetOption = CanvasPromptPreset & {
...
export function CanvasPresetPicker({ mode, skillReferences = [], open, onOpenChange, onSelect, compact = false, dense = false }: { mode: CanvasGenerationMode; skillReferences?: CanvasResourceReference[]; open?: boolean; onOpenChange?: (open: boolean) => void; onSelect: (preset: CanvasPromptPreset) => void; compact?: boolean; dense?: boolean }) {
    ...
    const presets = useMemo(() => {
        const skills = skillReferences.flatMap((reference): CanvasPromptPresetOption[] => {
        ...
        // 这批材料是生成“最终提示词”的元提示词，只在文本节点中提供，避免误投喂图片或视频模型。
        const mingwant = mode === "text" ? mingwantPromptTemplates.map((template): CanvasPromptPresetOption => ({
            id: template.id,
            name: template.name,
            ...
            modes: ["text"],
            source: "mingwant",
            loadPrompt: () => loadMingWantPrompt(template),
        })) : [];
        const normalized = query.trim().toLowerCase();
        return [...BUILTIN_PRESETS.filter((preset) => preset.modes.includes(mode)), ...mingwant, ...skills].filter((preset) => !normalized || `${preset.name} ${preset.description}`.toLowerCase().includes(normalized));
    }, [mode, query, skillReferences]);

    const selectPreset = async (option: CanvasPromptPresetOption) => {
        if (!option.loadPrompt) {
            onSelect(option);
            setOpen(false);
            return;
        }
        setLoadingPresetId(option.id);
        try {
            const prompt = await option.loadPrompt();
            const { loadPrompt: _loadPrompt, ...preset } = option;
    ...
    const content = (
        <div data-canvas-no-zoom className="w-[320px] max-w-[calc(100vw-32px)]" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
            <Input autoFocus allowClear size="small" prefix={<Search className="size-3.5 opacity-45" />} placeholder="搜索预设、明想库或技能" value={query} onChange={(event) => setQuery(event.target.value)} />
            {mode === "text" ? <div className="mt-2 rounded-md px-2 py-1.5 text-[10px] leading-4" style={{ background: theme.toolbar.itemHover, color: theme.node.muted }}>明想库模板会先由文本模型生成最终图片或视频提示词。</div> : null}
            <div className="thin-scrollbar mt-2 max-h-72 overflow-y-auto space-y-1">
                {presets.length ? presets.map((preset) => (
                    ...
                    >
                        <span className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-md" style={{ background: theme.toolbar.itemHover, color: theme.node.activeStroke }}>
                            {loadingPresetId === preset.id ? <LoaderCircle className="size-3.5 animate-spin" /> : preset.source === "mingwant" ? <BookOpenCheck className="size-3.5" /> : <WandSparkles className="size-3.5" />}
                        </span>
                        <span className="min-w-0 flex-1">
                            <span className="flex items-center gap-1.5 text-xs font-semibold" style={{ color: theme.node.text }}>
                                <span className="truncate">{preset.name}</span>
                                <span className="shrink-0 text-[9px] font-medium" style={{ color: theme.node.faint }}>{preset.source === "skill" ? "技能" : preset.source === "mingwant" ? "明想库" : "预设"}</span>
                            </span>
