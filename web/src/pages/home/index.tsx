...
const workflow = [
    { title: "锁定事实", description: "确认授权边界、商品信息和投放限制" },
    { title: "拆解素材", description: "从 5 条真人视频建立身份与商品锚点" },
    { title: "批量生产", description: "展开 15 条分镜、首尾帧和视频任务" },
    { title: "质检交付", description: "检查真实感、广告宣称、版本与结果" },
];
        ...
        <main className="app-user-content app-workspace-canvas h-full overflow-y-auto text-foreground">
            <div className="mx-auto w-full max-w-[1440px] px-4 pb-12 pt-5 sm:px-6 lg:px-8">
                {loadingUserWorkspace ? (
                    <WorkspaceLoadingState className="mt-3 max-w-[980px]" label="正在恢复工作台" detail="读取项目、素材和最近画布" rows={5} />
...
function ReturningWorkspace({ summary, detail, detailLoading, detailError, recentProjects, recentIndependentCanvases, onCreateIndependentCanvas }: {
    ...
    const commerceProject = summary.project.type === "commerce-video";
    const completedCount = commerceProject ? summary.completedWorkflowStepCount : summary.completedUnitCount;
    const totalCount = commerceProject ? summary.workflowStepCount : summary.unitCount;
    const unitLabel = commerceProject ? "工作流步骤" : "章";
    return (
        <>
            ...
            <section className="grid gap-0 border-b border-border/80 py-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
                <div className="min-w-0 py-2 lg:pr-8">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-foreground/52">
                        <span>{completedCount}/{totalCount} {unitLabel}已完成</span>
                    ...
                    <div className="mt-4 h-1.5 w-full max-w-[620px] overflow-hidden rounded-full bg-foreground/[.08]" aria-label={`${unitLabel}完成度 ${completion}%`}>
    ...
    return (
        <>
            <section className="border-b border-border/80 pb-8 pt-3 sm:pb-10 sm:pt-6">
                <div className="inline-flex items-center gap-2 text-xs font-semibold text-foreground/48"><WorkspaceSignalIcon variant="home" size="sm" />明想 MingWant Studio</div>
                <h1 className="mt-5 max-w-[820px] text-3xl font-semibold leading-[1.08] sm:text-4xl lg:text-5xl">把 5 条真人素材，推进为 15 条可投放的带货视频</h1>
            ...
            <section className="border-b border-border/80 py-7">
                <div className="mb-5"><h2 className="text-lg font-semibold">从授权素材到投放成片</h2><p className="mt-1 text-xs leading-5 text-foreground/48">每一步都保留输入、版本和生成记录，可以随时返回调整。</p></div>
            ...
            <section className="grid gap-8 py-7 lg:grid-cols-[minmax(0,1fr)_minmax(280px,.6fr)]">
                ...
                <div>
                    ...
                    <div className="mt-3 space-y-3 text-xs leading-5 text-foreground/52">
                        ...
                        <FeatureLine icon={<Bot className="size-4" />} text="Agent 读取你选择的脚本、节点和参考资料，再执行画布操作。" />
...
function RecentProjectRow({ summary, divided }: { summary: ProjectSummary; divided: boolean }) {
    ...
    const commerceProject = summary.project.type === "commerce-video";
    const totalCount = commerceProject ? summary.workflowStepCount : summary.unitCount;
    const unitLabel = commerceProject ? "步骤" : "章";
    return (
        <Link to={`/projects/${summary.project.id}/overview`} className={`group grid min-h-[68px] grid-cols-[minmax(0,1fr)_80px_20px] items-center gap-3 px-3 py-2.5 hover:bg-foreground/[.025] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-foreground/20 sm:grid-cols-[minmax(0,1fr)_100px_120px_20px] ${divided ? "border-t border-border/65" : ""}`}>
            <span className="min-w-0"><span className="block truncate text-sm font-medium">{summary.project.name}</span><span className="mt-1 block truncate text-[11px] text-foreground/42">{totalCount} {unitLabel} · {summary.canvasCount} 张项目画布 · {summary.assetCount} 项资产</span></span>
            <span className="hidden text-[11px] text-foreground/45 sm:block">更新于<br />{formatRelativeTime(summary.project.updatedAt)}</span>
            <span className="min-w-0"><span className="flex items-center justify-between text-[10px] text-foreground/42"><span>{commerceProject ? "工作流" : "章节"}</span><span>{completion}%</span></span><span className="mt-1.5 block h-1 overflow-hidden rounded-full bg-foreground/[.08]"><span className="block h-full rounded-full bg-foreground/65" style={{ width: `${completion}%` }} /></span></span>
